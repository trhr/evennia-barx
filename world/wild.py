...
def enter_wilderness(obj, coordinates=(0, 0), name="default"):
    ...
    """
    if not WildScript.objects.filter(db_key=name).exists():
        return False

    script = WildScript.objects.get(db_key=name)
...
def get_direction_from(from_coord, to_coord):
    x1, y1 = from_coord
    x2, y2 = to_coord
...
class WildScript(DefaultScript):
    """
...
class WildernessExit(DefaultExit):
    ...
    def at_traverse(self, traversing_object, target_location):
        ...
        traversing_object.location.db.travellers += [[traversing_object, current_coordinates, new_coordinates, datetime.now().strftime("%d %b %y %H:%M:%S")]]
        ...
        for traveller in self.location.db.travellers:
            if traveller[1] == new_coordinates:
               traversing_object.msg("{obj} headed {dir} {time}.".format(obj=traveller[0].get_sniff_name(traversing_object) , dir=get_direction_from(new_coordinates, traveller[2]), time=naturaltime(datetime.strptime(traveller[3], "%d %b %y %H:%M:%S"))))
